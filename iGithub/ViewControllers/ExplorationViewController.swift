...
class ExplorationViewController: BaseTableViewController {
    ...
    override func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .repositories:
            let repo = viewModel.repoTVM.repositories.value[indexPath.row]
...
extension ExplorationViewController: SegmentHeaderViewDelegate {
    func headerView(_: SegmentHeaderView, didSelectSegmentTitle title: TrendingType) {
...
extension ExplorationViewController: OptionPickerViewDelegate {
    ...
    func pickerView(_: UIPickerView, numberOfRowsInComponent _: Int) -> Int {
        return pickerView.index == 0 ? 3 : languagesArray.count
    }

    func pickerView(_: UIPickerView, titleForRow row: Int, forComponent _: Int) -> String? {
        return pickerView.index == 0 ? viewModel.timeOptions[row] : languagesArray[row]
    }
}
